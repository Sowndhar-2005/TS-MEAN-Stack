...
export class Order implements OnInit {
  ...
  payOrder() {
    ...
    const total = this.orderService.getSubtotal(); // Use subtotal for display/confirmation
    if (confirm(`Confirm payment of ₹${total}?`)) {
      if (this.orderService.confirmOrder()) {
  ...
  addListToCart() {
    ...
    // Note: updateGroup assigns ALL items with these IDs to the group.
    // This is a simplification; ideally we'd track the specific new instances.
    // For this user flow ("store all food list group"), this seems correct.
